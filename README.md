...
│                                                                   │
│   ┌──────────────────────┐     ┌───────────────────────────┐    │
│   │  football-data.org   │     │     transfermarkt.com      │    │
│   │       API v4         │     │       (scraping web)       │    │
│   │                      │     │                            │    │
│   │ • Classement         │     │ • Valeur marchande         │    │
│   │ • Matchs             │     │ • Taille effectif          │    │
│   │ • Équipes            │     │ • Âge moyen                │    │
│   │ • Buteurs            │     │ • Joueurs étrangers        │    │
│   └──────────┬───────────┘     └──────────────┬────────────┘    │
└──────────────┼──────────────────────────────────┼────────────────┘
               │                                  │
               ▼                                  ▼
...
│                          INGESTION                               │
│                                                                   │
│   ingestion/api_client.py          ingestion/scraper.py          │
│   • Appels HTTP + auth             • requests + BeautifulSoup    │
│   • Rate limit (6s/requête)        • Parsing HTML                │
│   • Retourne dict Python           • Retourne DataFrame          │
└──────────────────────────────┬──────────────────────────────────┘
...
│                        TRANSFORMATION                            │
│                                                                   │
│   transform_api.py                 transform_scraped.py          │
│   • JSON imbriqué → DataFrame      • Nettoyage types             │
│   • Renommage colonnes             • €1.21bn → 1210.0 (millions) │
│   • Typage (dates, entiers)        • Suppression NaN             │
│                                                                   │
│                         merge.py                                  │
│              • Fusion API + Transfermarkt                        │
│              • Correspondance noms d'équipes                     │
└──────────────────────────────┬──────────────────────────────────┘
                               │ DataFrames propres en mémoire
                               ▼
┌─────────────────────────────────────────────────────────────────┐
│                      DATA WAREHOUSE                              │
│                                                                   │
│   warehouse/load.py                                              │
│   • Connexion PostgreSQL (pg8000)                                │
│   • INSERT avec ON CONFLICT DO UPDATE (idempotent)               │
│   • Commit par table                                             │
│                                                                   │
│   ┌─────────────────────────────────────────────────────────┐   │
│   │              PostgreSQL (Docker)                         │   │
│   │                                                          │   │
│   │  competitions   teams      standings   matches           │   │
│   │  scorers        team_stats_scraped                       │   │
│   └─────────────────────────────────────────────────────────┘   │
└──────────────────────────────┬──────────────────────────────────┘
                               │ requêtes SQL
                               ▼
